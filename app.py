        ...
        for day in week:
            if day.month != month:
                ...
                if row.empty:
                    ...
                    z_row.append(signal_map.get(row.iloc[0]["Signal"], 0))
                    text_row.append(str(day.day))
        z.append(z_row)
...
st.plotly_chart(build_recent_signal_timeline(data, 30), width="stretch")
...
live_ledger = live_performance_ledger(data)
...
if live_ledger.empty:
    ...
    st.dataframe(breakdown.round(2), width="stretch")
    ...
    display_ledger = display_ledger[
        ["TradeDate", "DisplaySignal", "Confidence", "BuyPrice", "AmountInvested", "UnitsBought", "CurrentPrice", "CurrentValue", "ProfitLoss", "ReturnPct", "CurrentMarketDate"]
    ]
    st.dataframe(display_ledger.round(4), width="stretch")
...
st.subheader("Signal Calendar")
st.caption("Default view shows the last 3 months. Expand to 6 or 12 months using the selector.")

calendar_range = st.radio(
    "Calendar range",
...
cal_months = CALENDAR_OPTIONS[calendar_range]
cal_data = filter_months(data, cal_months).copy()
...
num_cols = calendar_columns_for_range(cal_months)

for i in range(0, len(month_periods), num_cols):
...
with history_tab1:
    ...
    signal_table["Signal"] = signal_table["DisplaySignal"]
    signal_table = signal_table[["Date", "Price", "RSI", "VIX", "DrawdownPct", "Signal", "Suggested Investment"]]
    st.dataframe(signal_table.round(2), width="stretch")
...
with history_tab2:
    ...
    full_table["Signal"] = full_table["DisplaySignal"]
    full_table = full_table[["Date", "Price", "RSI", "VIX", "DrawdownPct", "Signal", "Suggested Investment"]]
    st.dataframe(full_table.round(2), width="stretch")